...
def plot_sweep(filename, char, color, label_str, prev_label, index):
	...
	n = 1
	for i in neff:
		if (n+1 < len(neff) and n-1 > -1):
			fdash = (-neff[n+1] + neff[n-1]) / (-wavelength[n+1] + wavelength[n-1])
			gp = 1./(1.0 - (fdash*wavelength[n]))
			groupVel.append(gp)
			temp_wavelength.append(wavelength[n])
			print gp
			n+=1
	...
	if (index < number_lines):
		...
		ax1.plot(temp_wavelength, groupVel, color + '-', linewidth=2.0, label=label_str)
...
ax1.set_ylabel("Group velocity / c, $v_g / c$", fontsize=30)
#plt.xlim(0,12.4)
#plt.ylim(0,1.0)
...
ax1.legend()
plt.tight_layout()
plt.savefig('GroupVel' + label + '.eps', format='eps')
plt.savefig('GroupVel' + label + '.png', format='png', dpi=200)
plt.show()